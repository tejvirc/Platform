namespace Aristocrat.Monaco.Asp.Client.Comms
{
    public class AspCrc
    {
        ...
        public static ushort CalcCrc(byte currentByte, ushort prevCrc)
        {
            var aWord = (prevCrc << 8) & 0xFFFF;
            var aByte = (byte)((prevCrc >> 8) & 0xFF);
            prevCrc = (ushort)((aWord ^ CrcTable[(aByte ^ currentByte) & 0xFF]) & 0xFFFF);
            return prevCrc;
        }

        public static ushort CalcCrc(byte[] data, int offset = 0, int count = -1, ushort prevCrc = 0)
        {
            count = count < 0 ? data.Length - offset : count;
            for (var i = offset; i < offset + count; i++)
            {
                prevCrc = CalcCrc(data[i], prevCrc);
            }
