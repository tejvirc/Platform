namespace Aristocrat.Monaco.G2S.Consumers
{
    ...
    /// <summary>
    ///     Handles the <see cref="SecondaryGameStartedEvent" /> event.
    /// </summary>
    public class SecondaryGameStartedConsumer : GamePlayConsumerBase<SecondaryGameStartedEvent>
    {
        /// <summary>
        ...
        public SecondaryGameStartedConsumer(IG2SEgm egm, IEventLift eventLift, IGameMeterManager gameMeters)
            : base(egm, eventLift, gameMeters, EventCode.G2S_GPE109)
        {
        }
                        ...
                        new[]
                        {
                            new deviceMeters
                            {
                                ...
                                simpleMeter =
                                    GetSimpleMeters(
                                        device.Id,
                                        PerformanceMeterName.SecondaryWageredAmount).ToArray()
                            },
                            new deviceMeters
                            {
                                deviceClass = device.PrefixedDeviceClass(),
                                deviceId = 0,
                                simpleMeter =
                                    GetSimpleMeters(PerformanceMeterName.SecondaryWageredAmount).ToArray()
                            }
                        }
                }
            };
