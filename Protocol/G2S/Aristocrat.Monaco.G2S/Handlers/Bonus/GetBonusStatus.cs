namespace Aristocrat.Monaco.G2S.Handlers.Bonus
{
    ...
    public class GetBonusStatus : ICommandHandler<bonus, getBonusStatus>
    {
        private readonly ICommandBuilder<IBonusDevice, bonusStatus> _commandBuilder;
        private readonly IG2SEgm _egm;

        public GetBonusStatus(IG2SEgm egm, ICommandBuilder<IBonusDevice, bonusStatus> commandBuilder)
        {
            _egm = egm ?? throw new ArgumentNullException(nameof(egm));
            _commandBuilder = commandBuilder ?? throw new ArgumentNullException(nameof(commandBuilder));
        }

        public async Task<Error> Verify(ClassCommand<bonus, getBonusStatus> command)
        {
            return await Sanction.OwnerAndGuests<IBonusDevice>(_egm, command);
        }

        public async Task Handle(ClassCommand<bonus, getBonusStatus> command)
        {
