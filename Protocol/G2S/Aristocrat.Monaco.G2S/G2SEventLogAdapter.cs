namespace Aristocrat.Monaco.G2S
{
    ...
    public class G2SEventLogAdapter : BaseEventLogAdapter, ISubscribableEventLogAdapter
    {
        ...
        {
            var hasInternalEvent = !string.IsNullOrEmpty(eventLog.InternalEventType);
            var eventType = hasInternalEvent ? Type.GetType(eventLog.InternalEventType) : typeof(G2SEvent);

            var result = eventType?.Name ?? nameof(G2SEvent);
            if (!hasInternalEvent)
            {
