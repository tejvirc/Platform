    ...
    public abstract class PwmDeviceProtocol : IGdsCommunicator
    {
        ...
        protected PwmDeviceProtocol()
        {
            _overlapped = new NativeOverlapped()
            {
                OffsetLow = 0,
                OffsetHigh = 0,
                EventHandle = NativeMethods.CreateEvent(IntPtr.Zero,
        ...
        protected virtual (bool, ChangeRecord) ReadSync()
        {
            ...
            var outputPtr = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(ChangeRecord)));
            ...
            try
            {
                unsafe
                {
                    ...
                    if (success)
                    {
                        var record = (ChangeRecord)Marshal.PtrToStructure(outputPtr, typeof(ChangeRecord));
                        return (true, record);
                    }
                }
