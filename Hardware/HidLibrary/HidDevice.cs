namespace Vgt.Client12.Hardware.HidLibrary
{
    ...
    public class HidDevice : IHidDevice
    {
        ...
        {
            var readDelegate = new ReadDelegate(Read);
            return await Task<HidDeviceData>.Factory.FromAsync(
        ...
        {
            var readReportDelegate = new ReadReportDelegate(ReadReport);
            return await Task<HidReport>.Factory.FromAsync(
                readReportDelegate.BeginInvoke,
                readReportDelegate.EndInvoke,
                timeout,
                null);
        }
        ...
        public bool Write(byte[] data, int timeout)
        {
            ...
            // TODO: IsConnected is slow.  It does some device enumeration.  Not something we want to do in real-time.
            /*
            if (IsConnected)
            {
        ...
        {
            var writeDelegate = new WriteDelegate(Write);
            return await Task<bool>.Factory.FromAsync(
        ...
        public async Task<bool> WriteReportAsync(HidReport report, int timeout = 0)
        {
            var writeReportDelegate = new WriteReportDelegate(WriteReport);
            return await Task<bool>.Factory.FromAsync(
                writeReportDelegate.BeginInvoke,
                writeReportDelegate.EndInvoke,
                report,
                timeout,
                null);
        }
        ...
        public HidReport ReadReportSync(byte reportId)
        {
            ...
            var deviceData = new HidDeviceData(
                cmdBuffer,
                bSuccess ? HidDeviceData.ReadStatus.Success : HidDeviceData.ReadStatus.NoDataRead);
        ...
        ///     that do not support the interrupt transfers
        /// </summary>
        /// <param name="report">The outbound HID report</param>
        /// <returns>The result of the transfer request: true if successful otherwise false</returns>
        public bool WriteReportSync(HidReport report)
        {
            if (null == report)
            {
                throw new ArgumentException(
                    "The output report is null, it must be allocated before you call this method",
                    nameof(report));
            }
        ...
        private static HidDeviceCapabilities GetDeviceCapabilities(IntPtr hidHandle)
        {
            ...
            var dataPointer = default(IntPtr);

            if (NativeMethods.HidD_GetPreparsedData(hidHandle, ref dataPointer))
            {
                NativeMethods.HidP_GetCaps(dataPointer, ref capabilities);
                NativeMethods.HidD_FreePreparsedData(dataPointer);
            }
        ...
        private bool WriteData(byte[] data, int timeout)
        {
            ...
            var buffer = data; // CreateOutputBuffer();
            ...
            if (_deviceWriteMode == DeviceMode.Overlapped)
            {
                ...
                overlapped.EventHandle = NativeMethods.CreateEvent(
                    IntPtr.Zero,
                    Convert.ToInt32(false),
                    Convert.ToInt32(false),
                    "");

                if (!NativeMethods.WriteFile(Handle, buffer, (uint)buffer.Length, out var _, ref overlapped))
                {
                    // If the write fails, see if it is because IO is pending.
                    var errorCode = Marshal.GetLastWin32Error();

                    if (errorCode == NativeMethods.ERROR_IO_PENDING)
                    {
                        var waitStatus = NativeMethods.WaitForSingleObject(overlapped.EventHandle, overlapTimeout);
                        ...
                        switch (waitStatus)
                        {
                            case NativeMethods.WAIT_OBJECT_0:
                                NativeMethods.GetOverlappedResult(Handle, ref overlapped, out var _, false);
                                return true;
            ...
            if (Capabilities.InputReportByteLength > 0)
            {
                ...
                if (_deviceReadMode == DeviceMode.Overlapped)
                {
                    ...
                    try
                    {
                        ...
                        if (success)
                        {
                            status = HidDeviceData.ReadStatus
                                .Success; // No check here to see if bytesRead > 0 . Perhaps not necessary?
                        }
                        else
                        {
                            var result = NativeMethods.WaitForSingleObject(overlapped.EventHandle, overlapTimeout);
