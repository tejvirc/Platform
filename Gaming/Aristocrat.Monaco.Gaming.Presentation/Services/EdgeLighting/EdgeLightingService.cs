...
public sealed class EdgeLightingService : IEdgeLightingService, IDisposable
{
    ...
    private void SetOverrideEdgeLight()
    {
        ...
        {
            _dispatcher.Dispatch(new EdgeLightUpdateOverrideAction { CanOverrideEdgeLight  = true });
        }
        else
        {
            _dispatcher.Dispatch(new EdgeLightUpdateOverrideAction { CanOverrideEdgeLight = false });
        }
    }
}
