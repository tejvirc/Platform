...
*
*
* Refactored and Single Header/amalgamation file by Thomas Rizos @ Aristocrat Technologies Inc
* Close to zero dependencies, use HIVEX_IMPLEMENTATION for default std dependencies
* Otherwise, implement/typedef/define your own implementations or existing.
*
*/
#ifndef HIVEX_H_
#define HIVEX_H_
...
#define HIVEX_OPEN_MSGLVL_MASK (HIVEX_OPEN_VERBOSE|HIVEX_OPEN_DEBUG)
		...
		hivex_open(const char *filename, int flags)
	{
		...
		/* Check major version. */
		hivex_uint32_t major_ver = le32toh(h->hdr->major_ver);
		if (major_ver != 1) {
			hivex_printf(
...
#define GET_CHILDREN_NO_CHECK_NK 1
			...
			int flags)
	{
		...
		* ri-record.
		*/
		hivex_size_t subkey_lf = le32toh(nk->subkey_lf);
		subkey_lf += 0x1000;
	...
	*/
			...
			hive_value_h **values_ret, hivex_size_t **blocks_ret)
	{
		...
		hivex_size_t vlist_offset = le32toh(nk->vallist);
		vlist_offset += 0x1000;
		...
		hivex_size_t len = block_len(h, vlist_offset, NULL);
	...
	*/
	...
	HIVEX_DEF char * hivex_utf16_to_utf8(const  hivex_uint16_t  *src, hivex_size_t len)
	{
		...
		hivex_uint32_t code_high = 0;

		while (size--)
		{
			hivex_uint32_t code = *src++;
