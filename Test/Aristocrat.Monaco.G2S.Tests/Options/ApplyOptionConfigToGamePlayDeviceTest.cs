namespace Aristocrat.Monaco.G2S.Tests.Options
{
    ...
    [TestClass]
    public class ApplyOptionConfigToGamePlayDeviceTest : BaseApplyOptionConfigToDeviceTest
    {
        [TestMethod]
        public void WhenApplyExpectSuccess()
        {
            var device = new GamePlayDevice(DeviceId, _deviceObserverMock.Object);
            ...
            _egmMock.Setup(x => x.GetDevice(DeviceClass.G2S_gamePlay.TrimmedDeviceClass(), DeviceId))
                .Returns(device);
            ...
            var service = new ApplyOptionConfigToDevicesService(
                ...
                {
                    new GamePlayDeviceOptions(new Mock<IGameProvider>().Object)
                });

            service.UpdateDeviceProfiles(CreateChangeOptionConfigRequest(DeviceClass.G2S_gamePlay));
            ...
            _profileServiceMock.Verify(
                x => x.Save(It.Is<IDevice>(d => d.Id == DeviceId && d.DeviceClass == typeof(gamePlay).Name)),
                Times.Once);
        }
