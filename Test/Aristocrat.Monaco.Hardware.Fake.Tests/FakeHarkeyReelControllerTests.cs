namespace Aristocrat.Monaco.Hardware.Usb.Tests.ReelController
{
    ...
    public class FakeHarkeyReelControllerTests
    {
        ...
        {
            const int expectedCapabilities = 3;

            var controller = new FakeHarkeyReelController();
        ...
        public async Task InitializationSucceedsWhenCommunicatorIsOpenTest()
        {
            ...
            var communicator = new Mock<IHarkeyCommunicator>();
            var controller = new FakeHarkeyReelController();
            ...
            Assert.AreEqual(expectedCapabilities, capabilities.Count());
            Assert.IsTrue(controller.HasCapability<IReelSpinImplementation>());
            Assert.IsTrue(controller.HasCapability<IReelBrightnessImplementation>());
            Assert.IsTrue(controller.HasCapability<IReelLightingImplementation>());
        }
